...
const GOALS=[
  {label:'MRR Target',current:84320,target:100000,color:C.blue},
  {label:'New Customers',current:1247,target:1500,color:C.green},
  {label:'Churn Goal',current:2.4,target:2.0,color:C.amber,inverse:true},
];
const NOTIFS=[
...
const STYLES=`
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .card:hover{transform:scale(1.022)}.card{transition:transform 0.2s ease}
  ...
  .btn-blue:hover{filter:brightness(1.1)}.btn-blue{transition:filter 0.15s}
  ...
  return(
    <div style={{marginBottom:16}}>
      ...
      <div style={{position:'relative'}}>
        ...
        <input type={type==='password'&&show?'text':type} value={value} onChange={onChange} placeholder={placeholder} onFocus={()=>setFocused(true)} onBlur={()=>setFocused(false)}
        ...
        {type==='password'&&<button type="button" onClick={()=>setShow(!show)} style={{position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',color:C.t3,display:'flex',alignItems:'center'}}>
...
function Sidebar({page,setPage,userName,userEmail,onLogout}){
  ...
  const initials = userName ? userName.split(' ').map(n=>n[0]).join('').toUpperCase().slice(0,2) : 'AS';
  return(
      ...
      <div style={{flex:1,padding:'0 10px'}}>
        ...
        {nav.map(({id,label,Icon})=>{
          ...
          return(<button key={id} className="nh" onClick={()=>setPage(id)} style={{width:'100%',display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:8,border:a?'1px solid rgba(91,138,255,0.22)':'1px solid transparent',background:a?'rgba(91,138,255,0.11)':'transparent',color:a?C.t1:C.t2,fontSize:13,fontWeight:500,cursor:'pointer',textAlign:'left',marginBottom:2,position:'relative',transition:'background 0.15s,border-color 0.15s'}}>
            ...
            <Icon size={15} color={a?C.blue:C.t2} strokeWidth={2}/>{label}
          </button>);
        })}
      </div>
      <div style={{padding:'0 10px',borderTop:'1px solid rgba(255,255,255,0.05)',paddingTop:12}}>
        <button onClick={onLogout} className="nh" style={{width:'100%',display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:8,border:'none',background:'transparent',color:C.t2,fontSize:13,fontWeight:500,cursor:'pointer',transition:'background 0.15s',marginBottom:8,fontFamily:'"DM Sans",sans-serif'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10,padding:12,background:'rgba(255,255,255,0.03)',borderRadius:10}}>
          <div style={{width:32,height:32,borderRadius:'50%',background:'linear-gradient(135deg,rgba(91,138,255,0.2),rgba(0,217,143,0.2))',border:'1px solid rgba(91,138,255,0.3)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:C.blue,fontFamily:'JetBrains Mono',flexShrink:0}}>{initials}</div>
          <div style={{minWidth:0,flex:1}}>
            <div style={{fontSize:12,fontWeight:600,color:C.t1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{userName||'User'}</div>
            <div style={{fontSize:10,color:C.t3,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{userEmail||''}</div>
          </div>
...
function TopBar({userName}){
  ...
  return(
    <div style={{position:'sticky',top:0,zIndex:50,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 32px',background:'rgba(7,7,14,0.85)',backdropFilter:'blur(12px)',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
      <div>
        <span style={{fontFamily:'Syne',fontSize:13,fontWeight:600,color:C.t2}}>{getGreeting()}, <span style={{color:C.t1}}>{userName||'there'}</span></span>
          ...
          <div style={{position:'absolute',top:42,right:0,width:290,background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:'14px 16px',boxShadow:'0 20px 60px rgba(0,0,0,0.5)',zIndex:100}}>
            ...
            {NOTIFS.map((n,i)=>(
              <div key={i} style={{display:'flex',gap:10,padding:'10px 0',borderTop:`1px solid ${C.border}`}}>
                ...
                <div><div style={{fontSize:12,color:C.t1,lineHeight:1.4}}>{n.title}</div><div style={{fontSize:11,color:C.t3,marginTop:3}}>{n.time}</div></div>
              </div>
            ))}
...
function MiniSparkline({data,color}){return(<div style={{position:'absolute',bottom:4,right:4,width:72,height:36,opacity:0.45,pointerEvents:'none'}}><ResponsiveContainer width="100%" height="100%"><LineChart data={data}><Line type="monotone" dataKey="v" stroke={color} strokeWidth={1.5} dot={false} animationDuration={1200}/></LineChart></ResponsiveContainer></div>);}
      ...
      <div style={{position:'relative'}}>
        ...
        <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',gap:8}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:4,fontSize:11,fontWeight:700,padding:'4px 8px',borderRadius:20,flexShrink:0,background:isGood?'rgba(0,217,143,0.12)':'rgba(255,90,110,0.12)',color:isGood?C.green:C.coral}}>
            {isGood?<TrendingUp size={10} strokeWidth={2.5}/>:<TrendingDown size={10} strokeWidth={2.5}/>}{Math.abs(change)}%
          </div>
        </div>
...
function GoalsWidget(){
  ...
  return(
    <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}>
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:18}}><Target size={15} color={C.blue}/><span style={{fontFamily:'Syne',fontWeight:600,fontSize:15,color:C.t1}}>Monthly Goals</span></div>
      {GOALS.map((g,i)=>{
        ...
        return(<div key={i} style={{marginBottom:i<GOALS.length-1?16:0}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
            <span style={{fontSize:12,color:C.t2}}>{g.label}</span>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              <span style={{fontFamily:'JetBrains Mono',fontSize:12,color:C.t1}}>{typeof g.current==='number'&&g.current>100?g.current.toLocaleString():g.current}</span>
              <span style={{fontSize:11,color:C.t3}}>/ {typeof g.target==='number'&&g.target>100?g.target.toLocaleString():g.target}</span>
...
function RevenueChart({data}){
  const [period,setPeriod]=useState('12M');const d=period==='6M'?data.slice(-6):data;
  return(<div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:24}}>
    <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:24}}>
      ...
      <div style={{display:'flex',gap:4,padding:4,background:'rgba(255,255,255,0.04)',borderRadius:8}}>{['6M','12M'].map(p=><button key={p} onClick={()=>setPeriod(p)} style={{padding:'6px 12px',borderRadius:6,border:'none',cursor:'pointer',fontSize:12,fontWeight:600,background:period===p?'rgba(255,255,255,0.09)':'transparent',color:period===p?C.t1:C.t2,transition:'background 0.15s',fontFamily:'"DM Sans",sans-serif'}}>{p}</button>)}</div>
    </div>
...
function OverviewPage({setPage}){
  return(<div style={{padding:32}}>
    ...
    <div className="up" style={{animationDelay:'0.52s',marginBottom:14}}><GoalsWidget/></div>
...
function PlanCard({plan,index}){const val=useCountUp(plan.mrr,1800,600+index*100);const pct=Math.round((plan.mrr/115200)*100);return(<div className="up card" style={{animationDelay:`${0.6+index*0.1}s`,background:C.bg,border:`1px solid ${plan.color}30`,borderRadius:12,padding:18}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}><span style={{fontSize:11,fontWeight:700,padding:'4px 10px',borderRadius:6,background:`${plan.color}18`,color:plan.color}}>{plan.plan}</span><span style={{fontSize:11,color:C.t3}}>{plan.customers} customers</span></div><div style={{fontFamily:'JetBrains Mono',fontSize:20,fontWeight:500,color:C.t1,marginBottom:3}}>${Math.round(val).toLocaleString()}</div><div style={{fontSize:11,color:C.t3,marginBottom:10}}>Monthly revenue</div><div style={{height:3,background:'rgba(255,255,255,0.05)',borderRadius:3,overflow:'hidden'}}><div style={{height:'100%',background:plan.color,borderRadius:3,width:`${pct}%`,transition:'width 1s cubic-bezier(0.16,1,0.3,1)'}}/></div></div>);}
...
function RevenuePage(){
  ...
  return(<div style={{padding:32}}>
    ...
    <div className="up" style={{animationDelay:'0.38s',background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:24,marginBottom:20}}>
      ...
      <ResponsiveContainer width="100%" height={190}><BarChart data={stackedData} barGap={3} margin={{top:4,right:8,left:0,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" vertical={false}/><XAxis dataKey="month" tick={{fill:'#7070A0',fontSize:12}} axisLine={false} tickLine={false}/><YAxis tick={{fill:'#7070A0',fontSize:12}} axisLine={false} tickLine={false} tickFormatter={v=>`$${(v/1000).toFixed(0)}k`} width={40}/><Tooltip content={<BarTip/>} cursor={{fill:'rgba(255,255,255,0.025)'}}/><Bar dataKey="New MRR" fill={C.blue} radius={[3,3,0,0]} maxBarSize={14}/><Bar dataKey="Expansion" fill={C.green} radius={[3,3,0,0]} maxBarSize={14}/><Bar dataKey="Churn" fill={C.coral} radius={[3,3,0,0]} maxBarSize={14}/></BarChart></ResponsiveContainer>
    </div>
...
function TransactionsPage(){
  ...
  return(<div style={{padding:32}}>
    ...
    <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,overflow:'hidden'}}>
      <div style={{display:'grid',gridTemplateColumns:'32px 1fr 86px 76px 78px 68px',gap:12,padding:'10px 16px',borderBottom:`1px solid ${C.border}`,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'0.1em',color:C.t3}}><div/><div>Customer</div><div>ID</div><div>Plan</div><div>Status</div><div style={{textAlign:'right'}}>Amount</div></div>
...
function SettingsPage({userName,userEmail}){
  ...
  return(<div style={{padding:32,maxWidth:680}}>
    ...
    <div className="up" style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:24,marginBottom:16}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:24,padding:16,background:'rgba(255,255,255,0.03)',borderRadius:10}}>
        <div style={{width:56,height:56,borderRadius:'50%',background:'linear-gradient(135deg,rgba(91,138,255,0.3),rgba(0,217,143,0.3))',border:'2px solid rgba(91,138,255,0.4)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:700,color:C.blue,fontFamily:'JetBrains Mono',flexShrink:0}}>{name?name.split(' ').map(n=>n[0]).join('').toUpperCase().slice(0,2):'AS'}</div>
        <div><div style={{fontSize:15,fontWeight:600,color:C.t1,marginBottom:2}}>{name||'User'}</div><div style={{fontSize:12,color:C.t2}}>{email}</div><div style={{fontSize:11,color:C.t3,marginTop:4}}>Admin · {company}</div></div>
      ...
      <button onClick={handleSave} className="btn-blue" style={{marginTop:16,padding:'10px 20px',borderRadius:8,border:'none',cursor:'pointer',background:saved?'rgba(0,217,143,0.2)':'linear-gradient(135deg,#5B8AFF,#4A6FE8)',color:saved?C.green:'white',fontSize:13,fontWeight:600,fontFamily:'"DM Sans",sans-serif',border:saved?`1px solid ${C.green}`:'none',transition:'all 0.2s'}}>
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: C.bg, fontFamily: '"DM Sans",system-ui,sans-serif', backgroundImage: 'radial-gradient(rgba(255,255,255,0.022) 1px,transparent 1px)', backgroundSize: '28px 28px' }}>
        <Sidebar page={page} setPage={setPage} userName={userName} userEmail={userEmail} onLogout={handleLogout} />
